...
%end
function im_unwrapped = GoldsteinUnwrap(U,im_mask)
im_mag   = abs(U);                  %Magnitude image
im_phase = angle(U);                %Phase image
...
if(~exist('im_mask','var'))
  im_mask = ones(size(U));          %Mask (if applicable)
end
...
%else   % Chose starting point = max. intensity
  ...
  im_mag1(:,1) = 0;
  im_mag1(:,c_dim) = 0;
  [rowrefn,colrefn] = find(im_mag1 >= 0.85*mag_max);
