function phz = instance_phase_screen(r0, N, delta, L0, l0,seed)
    ...
    PSD_phi = 0.023*r0^(-5/3) * exp(-(f/fm).^2) ./ (f.^2 + f0^2).^(11.0/6);
    PSD_phi(N/2+1,N/2+1) = 0;
    % random draws of Fourier coefficients
    rng(seed)
    cn = (randn(N) + 1i*randn(N)) .* sqrt(PSD_phi)*del_f; % synthesize the phase screen
    phz = real(ift2(cn,1));
    %phz = mat2gray(phz);
    %imshow(phz);
    %colorbar;