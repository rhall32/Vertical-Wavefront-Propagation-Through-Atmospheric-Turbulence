function phz_com = ft_sh_phase_screen(r0, N, delta, L0, l0)
...
% loop over
%frequency grids with spacing 1/(3^p*L)
%the PSD
for p=1:3 % setup 
    del_f = 1 / (3^p*D); %frequency grid spacing [1/m] 
    fx = (-1 : 1) * del_f; % frequency grid [1/m]
    [fx, fy] = meshgrid(fx);
    [th, f] = cart2pol(fx, fy); % polar grid
    fm = 5.92/l0/(2*pi); % inner scale frequency [1/m] 
    f0 = 1/L0; % outer scale frequency [1/m] % modified von Karman atmospheric phase PSD 
...
% loop over frequencies on this grid
    for ii = 1:9
        SH = SH + cn(ii) * exp(1i*2*pi*(fx(ii)*x+fy(ii)*y)); phz_lo = phz_lo + SH; % accumulate subharmonics
    end
end
phz_lo = real(phz_lo) - mean(real(phz_lo(:)));
phz_com = phz_lo + phz_hi;
phz_com = mat2gray(phz_com);
%imshow(phz_com);
%colorbar;